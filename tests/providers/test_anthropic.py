...
async def test_anthropic_provider():
    ...
    expected_client_cls = httpx2.AsyncClient if anthropic_uses_httpx2() else httpx.AsyncClient
    assert isinstance(provider.client._client, expected_client_cls)  # pyright: ignore[reportPrivateUsage]
    async with provider:
        pass
    assert provider.client._client.is_closed  # pyright: ignore[reportPrivateUsage]


async def test_anthropic_provider_pass_http_client() -> None:
    client_cls = httpx2.AsyncClient if anthropic_uses_httpx2() else httpx.AsyncClient
    async with client_cls() as http_client:
...
async def test_anthropic_provider_rejects_other_flavor_http_client() -> None:
    # The SDK accepts exactly one HTTP flavor (httpx2 on 1.x, legacy httpx on 0.x) and refuses the other
    # at construction, so there is no deprecation path to offer.
    other_cls = httpx.AsyncClient if anthropic_uses_httpx2() else httpx2.AsyncClient
    async with other_cls() as http_client:
