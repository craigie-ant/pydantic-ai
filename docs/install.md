...
Providers whose SDKs still use legacy `httpx` (such as Groq and Cohere, and Anthropic before `anthropic` 1.0) keep its `certifi`-based verification and take a legacy `httpx.AsyncClient` instead.
